...
import { crypto } from '../provider/crypto.js'
import { issuer } from '../provider/issuer.js'
...
export const plugins = pluginify(async (server: FastifyInstance) => {
	...
	// Decorate the server instance with the database provider, crypto and issuer
	// services.
	server.decorate('database', database)
	server.decorate('crypto', crypto)
	server.decorate('issuer', issuer)
